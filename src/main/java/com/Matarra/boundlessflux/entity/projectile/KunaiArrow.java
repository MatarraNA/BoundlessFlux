...
public class KunaiArrow extends AbstractArrow implements IBoundlessArrow
{
    ...
    public KunaiArrow(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.referenceItem = ModItems.KUNAI_ARROW.get();
    }

    public KunaiArrow(LivingEntity pShooter, Level pLevel, Item referenceItem) {
        super(ModEntities.KUNAI_ARROW.get(), pShooter, pLevel);
        this.referenceItem = referenceItem;
    }

    @Override
    protected SoundEvent getDefaultHitGroundSoundEvent() {
        return ModSounds.SHURIKEN_HIT.get();
    }
    ...
    protected void onHit(HitResult pResult) {
        ...
        if( !this.level.isClientSide )
        {
            if( isThorArrow )
            {
                BoundlessLightning lightning = ModEntities.BOUNDLESS_LIGHTNING_AQUA.get().spawn((ServerLevel) this.getLevel(),
